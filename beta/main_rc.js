window.addEventListener('DOMContentLoaded', () => {
  ...
  function importLayout(parsed) {
    ...
    if (parsed.buttons) {
      Object.entries(parsed.buttons).forEach(([k, data]) => {
        ...
        Object.assign(appState.buttons[k], data);
  ...
  function openColorPanel(anchorTarget, x, y) {
    ...
    const innerSlider = document.createElement('input'); innerSlider.type = 'range'; innerSlider.min = 0; innerSlider.max = 15; innerSlider.step = 1; innerSlider.style.width = '80px';
    const innerValue = document.createElement('span');
    const outerLabel = document.createElement('span'); outerLabel.textContent = 'Out';
    const outerSlider = document.createElement('input'); outerSlider.type = 'range'; outerSlider.min = 0; outerSlider.max = 15; outerSlider.step = 1; outerSlider.style.width = '80px';
